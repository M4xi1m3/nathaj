...
export class Layout {
     ...
     */
    private static random(net: Network, minX = -100, maxX = 100, minY = -100, maxY = 100) {
        net.getDevices().forEach((dev) => {
            dev.setPosition(new Vector2D(Math.random() * (maxX - minX) + minX, Math.random() * (maxY - minY) + minY));
        });
    }
     ...
     */
    private static fruchterman_reingold(
        net: Network,
        epsilon = 0.01,
        max_iter = 1000,
        ideal_length = 100,
        ...
        const apply_temperature = (force: Vector2D, temp: number) => {
            return force
                .normalize()
                .div(force.length())
                .mul(Math.min(force.length(), temp * ideal_length * 2));
        };
        ...
        while (iter < max_iter && max_force > epsilon) {
            ...
            net.getDevices().forEach((u_dev) => {
                ...
                net.getDevices().forEach((v_dev) => {
                    if (u_dev === v_dev) return;
                    forces[u_dev.getName()] = forces[u_dev.getName()].add(
                        repulsive_force(u_dev.getPosition(), v_dev.getPosition())
                    );
                });
                ...
                u_dev
                    .getInterfaces()
                    .map((intf) => intf.getConnection()?.getOwner())
                    .forEach((v_dev) => {
                        if (v_dev === undefined) return;
                        if (u_dev === v_dev) return;

                        forces[u_dev.getName()] = forces[u_dev.getName()].add(
                            attractive_force(u_dev.getPosition(), v_dev.getPosition())
                        );
                    });
            });
            ...
            for (const u_dev of net.getDevices()) {
                if (Number.isNaN(forces[u_dev.getName()].x) || Number.isNaN(forces[u_dev.getName()].y)) return;
